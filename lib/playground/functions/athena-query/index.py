...
# Add the parent directory to the path so we can import the athena_tool module
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from tools.athena_tool import AthenaQueryTool
    ...
    try:
        ...
        if action == "execute_query":
            ...
            if not database:
                return {
                    "status": "error",
                    "content": [{"text": "Database is required for execute_query action"}],
                }
            ...
            if not database:
                return {
                    "status": "error",
                    "content": [{"text": "Database is required for get_table_schema action"}],
                }
            
            if not table:
                return {
                    "status": "error",
                    "content": [{"text": "Table is required for get_table_schema action"}],
                }
            ...
            if not result["success"]:
                return {
                    "status": "error",
                    "content": [{"text": f"Error getting table schema: {result['error']}"}],
                }
